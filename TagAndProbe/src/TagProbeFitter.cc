...
TagProbeFitter::TagProbeFitter(vector<string> inputFileNames, vector<string> inputFileNamesBkg, string inputDirectoryName, string inputTreeName, string outputFileName, int numCPU_, bool saveWorkspace_, bool floatShapeParameters_, std::vector<std::string> fixVars_){
  inputTree = new TChain((inputDirectoryName+"/"+inputTreeName).c_str());
  for(size_t i=0; i<inputFileNames.size(); i++){
    inputTree->Add(inputFileNames[i].c_str());
  }
  inputTreeBkg = new TChain((inputDirectoryName+"/"+inputTreeName).c_str());
  for(size_t i=0; i<inputFileNamesBkg.size(); i++){
    inputTreeBkg->Add(inputFileNamesBkg[i].c_str());
  }
...
string TagProbeFitter::calculateEfficiency(string dirName, vector<string> effCats, vector<string> effStates, vector<string>& unbinnedVariables, map<string, vector<double> >& binnedReals, map<string, std::vector<string> >& binnedCategories, vector<string>& binToPDFmap){
  ...
  RooDataSet dataBkg("dataBkg", "dataBkg", inputTreeBkg, 
                  dataVars,
                  /*selExpr=*/"", /*wgtVarName=*/(weightVar.empty() ? 0 : weightVar.c_str()));
  if (DATA != 1) data.append(dataBkg);
...
//      RooRealVar *colBkg = (RooRealVar *) dataBkg.addColumn(expr);
...
//      RooCategory *catBkg = (RooCategory *) dataBkg.addColumn(tmp);
  ...
  if (effCats.size() == 1) {
      ...
      dataBkg.addColumn( efficiencyCategory );
  } else {
...
//  RooDataSet sbsEfficiency("sbs_eff", "Efficiency from side band substraction", RooArgSet(RooArgSet(binnedVariables, categories), efficiency), StoreAsymError(RooArgSet(binnedVariables, efficiency)));
  ...
  if(!floatShapeParameters){
    ...
    doFitEfficiency(w, pdfCategory.getLabel(), efficiency, &dataBkg);
    delete w;
  }
  ...
  for(RooCatType* t = (RooCatType*)it->Next(); t!=0; t = (RooCatType*)it->Next() ){
    ...
    RooDataSet* data_bin_bkg = (RooDataSet*) dataBkg.reduce(//SelectVars(RooArgSet(variables["mass"], variables["passing"])),
      Cut(TString::Format("allCats==%d",t->getVal())));
    ...
    if(data_bin->numEntries()>0){
      ...
      efficiency.setVal(0);//reset
      efficiency.setAsymError(0,0);
      doFitEfficiency(w, pdfName.Data(), efficiency, data_bin_bkg);
...
void TagProbeFitter::doFitEfficiency(RooWorkspace* w, string pdfName, RooRealVar& efficiency, RooDataSet* dataBkg){
  ...
  if(!fixVars.empty()){
    ...
    if(!floatShapeParameters && fixVarValues.empty()){
      ...
      varFixer(w,true);
      //do fit 
      w->pdf("simPdf")->fitTo(*data, Save(true), Extended(true), NumCPU(numCPU), PrintLevel(quiet?-1:1), PrintEvalErrors(quiet?-1:1), Warnings(!quiet), SumW2Error(kTRUE));
      //release vars
      varFixer(w,false);
      //do fit 
      w->pdf("simPdf")->fitTo(*data, Save(true), Extended(true), NumCPU(numCPU), PrintLevel(quiet?-1:1), PrintEvalErrors(quiet?-1:1), Warnings(!quiet), SumW2Error(kTRUE));
    ...
    varFixer(w,true);
    //do fit
    res = w->pdf("simPdf")->fitTo(*data, Save(true), Extended(true), NumCPU(numCPU), Minos(*w->var("efficiency")), PrintLevel(quiet?-1:1), PrintEvalErrors(quiet?-1:1), Warnings(!quiet), SumW2Error(kTRUE));
  }//if(!fixVars.empty())
  ...
  if(floatShapeParameters){
    ...
    //do fit
    res = w->pdf("simPdf")->fitTo(*data, Save(true), Extended(true), NumCPU(numCPU), /*Minos(*w->var("efficiency"))*/Minos(false), PrintLevel(quiet?-1:1), PrintEvalErrors(quiet?-1:1), Warnings(!quiet), SumW2Error(kTRUE));
  }
...
void TagProbeFitter::saveFitPlot(RooWorkspace* w, RooDataSet* dataBkg){
...
//   RooAbsData* dataFail = dataAll->reduce(Cut("_efficiencyCategory_==_efficiencyCategory_::Failed")); 
  ...
  RooAbsData* dataPass = dataAll->reduce(Cut("_efficiencyCategory_==_efficiencyCategory_::Passed")); 
  RooAbsData* dataPassBkg = dataBkg->reduce(Cut("_efficiencyCategory_==_efficiencyCategory_::Passed")); 
  RooAbsData* dataFail = dataAll->reduce(Cut("_efficiencyCategory_==_efficiencyCategory_::Failed")); 
  RooAbsData* dataFailBkg = dataBkg->reduce(Cut("_efficiencyCategory_==_efficiencyCategory_::Failed")); 
  ...
  if (DATA != 1) dataPassBkg->plotOn(frames[0], MarkerColor(9), LineColor(9));
  ...
  canvas.cd(2);
  if (DATA != 1) dataFailBkg->plotOn(frames[1], MarkerColor(9), LineColor(9));
  ...
  // plot the All Probes
  canvas.cd(3);
  if (DATA != 1) dataBkg->plotOn(frames.back(), MarkerColor(9), LineColor(9));
