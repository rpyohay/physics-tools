...
class TagProbeFitter {
  ...
  TagProbeFitter(std::vector<std::string> inputFileNames, std::vector<std::string> inputFileNamesBkg, std::string inputDirectoryName, std::string inputTreeName, std::string outputFileName, int numCPU = 1, bool saveWorkspace_ = false, bool floatShapeParameters = true, std::vector<std::string> fixVars_ = std::vector<std::string>() );
  ...
  void doFitEfficiency(RooWorkspace* w, std::string pdfName, RooRealVar& efficiency, RooDataSet* dataBkg);
  ...
  void saveFitPlot(RooWorkspace* w, RooDataSet* dataBkg);
